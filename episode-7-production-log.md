...
## Music round 4: no music on clip1/clip9, overall volume down (2026-08-29)

Owner: remove music entirely from clip1 (opening thesis) and the horse-joke campfire scene (clip9) -- both play better dry, a clean cold-open hook and a dense multi-voice comedy beat don't want a bed under them. Also: the music overall is too loud.

**Fix, in `mix_music.mjs`**: base score volume cut 0.55->0.32 (~-4.7dB) and sidechain duck ratio deepened 6:1->9:1, so the score sits further under dialogue everywhere, not just in the two muted clips. Added `MUTE_WINDOWS` -- two `volume=0:enable=between(t,start,end)` filters gating the score to true silence across clip1's [0, 9.041667] and clip9's [83.333336, 91.375003] absolute-timeline spans, chained as independent filters (not summed into one expression -- the compound-sum form was the thing that broke during the original ducking build; single `enable=between()` gates are ffmpeg's standard, well-tested mechanism for exactly this and were verified correct in isolation via a throwaway script before trusting them here, since execFile passes argv with no shell and the comma inside `between(t,a,b)` needs its own literal-backslash escape to survive ffmpeg's filtergraph grammar rather than a shell's).
...
Owner reported the text wasn't visible and asked for it moved down to
just above her head. Re-checked the clean source frame directly rather
than guessing: her hairline sits noticeably lower than the first pass
assumed, leaving a large unused gap between the text and her head.
Re-burned at y=280/400 (bottom edge ~520px, clear of even the tallest
wind-blown hair strands), sitting immediately above her head instead of
high in the smoke. Re-delivered.
