...
5. **Before presenting any final prompt, run this checklist and fix anything that fails before finalizing, not after a poor generation reveals the gap:**
   ...
   - **If a beat's dialogue is written under the plain-language lock (literal, flat words), have you separately asked whether irony/sarcasm/dark humor still belongs in that beat via FACE and CUT only (playbook Part 14: irony lives in expression, not word choice — the two are compatible, not mutually exclusive)? "Plain-language words" must never be allowed to silently collapse into "flat, humorless performance" — check face/expression as its own line item, not as a byproduct of the words being literal.** (Concrete failure case, logged 2026-08-30: Escobar 7.1c's "Silver is money. Lead is a bullet." was written with the correct plain-language words but the prompt also specified "flat, zero irony" for her EXPRESSION, erasing the dark-history-deadpan irony the beat was written to carry — the owner caught that her performance read as merely literal instead of dryly, darkly amused. See `episode-escobar-production-log.md`.)
