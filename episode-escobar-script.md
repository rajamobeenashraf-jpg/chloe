...
hard-cut together, cuts every 2–10s, shot-reverse-shot dialogue, crash-zoom
...
- Each shot = one generation OR a digital reframe of a sibling take (crash
  zooms/punch-ins are edit-stage digital zooms — her own method — so one generated
  take can yield 2 shots).
- **No action is ever rendered twice from two angles** (S12/§8.2): dialogue
  coverage alternates singles at line boundaries; inserts show what the master
  doesn't; reactions are their own beats. Cut-on-action, never re-shown action.
- Adjacent shots chain by end-state/first-frame declarations; every scene's shots
  share the Episode Style Prefix + reference package. All cuts TRUE HARD CUTS,
...
voice lock · wpm table (v4 values hold; shot splits change coverage, not timing) ·
pacing refs measured per character per clip · `{}` dialogue syntax · rifle filter
fallback (grip-only, owner-consulted first) · ⛓ chaining · deliver-on-render ·
ask-before-regenerate · no 4K pre-approval · no drugs on screen · violence spoken,
never shown · facts as verified.
