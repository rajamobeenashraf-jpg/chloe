...
| N9 | **A denim tube-top + mini-skirt outfit on the identity-locked Hazel sits ON the `nano_banana_2` NSFW filter's boundary — it passes sometimes and fails sometimes, on essentially the same prompt.** Found 2026-09-01 (Spartacus arena): the same outfit rendered fine twice (`7abde812` tank+jeans, `a95a0b7b` denim set) and then failed three times running while trying to add garment specificity — `2ec0592d` and `54119ee6` (both carried a cropped product photo as a 6th reference), then `dea5405c` (text-only, returned an explicit **`nsfw`** status), then `f0ea11c9` (text-only, de-risked wording, still failed). Two separate lessons: **(a)** passing a real product photo of a DIFFERENT partially-clothed woman as a wardrobe reference alongside an identity-locked subject reliably fails — use the picture to inform Claude's own written description instead, which is strictly what the owner's reference picture is for; **(b)** wording changes did not reliably move it, so treat this as probabilistic, not a prompt bug — do NOT iterate blind. Stop after one retry and raise it. **Forward risk, flagged before it bites:** the shots this episode still needs (S15–S18: a woman on the ground, a man standing over her with a weapon) combine that outfit with a position the filter weighs much more heavily, and video generation applies its own filter — expect repeated, unpredictable failures on exactly the shots the copy cannot omit. | `2ec0592d`, `54119ee6`, `dea5405c` (nsfw), `f0ea11c9` vs. passing `7abde812`, `a95a0b7b` | 2026-09-01 |
