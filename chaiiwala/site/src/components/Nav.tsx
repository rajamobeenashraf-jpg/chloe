    ...
    <>
      <motion.header initial={{ y: -80, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 1, ease: [0.16, 1, 0.3, 1], delay: 0.2 }}
        ...
        <div className={`mx-auto flex max-w-7xl items-center justify-between rounded-full px-4 py-2.5 transition-all duration-500 ${solid ? "bg-cream/80 backdrop-blur-xl shadow-[0_10px_40px_-20px_rgba(42,23,16,.4)] border border-ink/5" : "bg-transparent"}`}>
          <Link href="/" className="font-display text-2xl font-black tracking-tight lowercase">chaii<span className="text-orange">wala</span><span className="align-super text-[10px]">®</span></Link>
          <nav className="hidden items-center gap-1 md:flex">
            {links.map((l) => (
              <Link key={l.href} href={l.href} className="rounded-full px-4 py-2 text-sm font-semibold text-ink/80 transition hover:bg-ink/5 hover:text-ink">{l.label}</Link>
            ))}
          </nav>
          <div className="flex items-center gap-2">
            <Magnetic><Link href="/menu" className="btn btn-primary hidden md:inline-flex">Order now</Link></Magnetic>
            <button aria-label="Menu" onClick={() => setOpen(true)} className="grid size-11 place-items-center rounded-full border border-ink/10 md:hidden">
