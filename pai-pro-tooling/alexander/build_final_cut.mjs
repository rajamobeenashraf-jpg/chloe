// Alexander/Gaugamela episode — final cut assembly. Every transition is a
// TRUE HARD CUT (creative-direction.md §16); no title cards (OPENING LAW).
// Adapted from pai-pro-tooling/troy/build_final_cut.mjs.
...
async function main() {
  ...
  const masterPath = path.join(ASSETS_DIR, "alexander_final_cut.mp4");

  // Re-encode (not stream-copy concat) so every clip's slightly different
  // encode params normalize into one consistent output.
  ...
  const compressedPath = path.join(ASSETS_DIR, "alexander_final_cut_compressed.mp4");
  await run("ffmpeg", [
    "-y", "-i", masterPath,
