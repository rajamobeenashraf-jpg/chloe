// Alexander/Gaugamela episode — QC pass: burn in synced captions (timing
// grounded in real silencedetect + faster-whisper word timing, see
// make_word_chunks.py), loudnorm each clip's audio, and apply a short
// audio-only fade in/out at every clip's edges so the hard cuts in
// build_final_cut.mjs don't snap audio at full volume. Adapted from
// pai-pro-tooling/troy/qc_pass.mjs per creative-direction.md §16.
...
async function qcOneClip(clip) {
  ...
  // Every clip normalizes to 1080x1920 8-bit yuv420p/h264 here, whatever its
  // source resolution/codec (Higgsfield returned 5a-aerial/5a/5b at 720x1280
  // and 5c/6a/6b as 10-bit HEVC, vs. everything else at 1080x1920 8-bit
  // h264) -- concat's filter_complex fails ("Failed to inject frame into
  // filter network: Invalid argument" / "Error reinitializing filters!")
  // when resolution, pixel format, or codec differ across inputs.
  const SCALE = "scale=1080:1920:force_original_aspect_ratio=decrease,pad=1080:1920:(ow-iw)/2:(oh-ih)/2";

  if (clip.captions.length === 0) {
    console.log(`[qc] ${clip.id}: no captions, loudnorm + fade + normalize`);
    await run("ffmpeg", [
      "-y", "-i", srcPath,
      "-vf", SCALE,
