// Episode 8 (Berlin Wall, 1989) — QC pass: burn in synced captions (timing
// grounded in real ffmpeg silencedetect boundaries + contact-sheet mouth
// checks on the ambiguous clips — see captions_data.mjs header), loudnorm
// each clip's audio, and apply a short audio-only fade in/out at every
// clip's edges so the hard cuts in build_final_cut.mjs don't snap audio at
// full volume. Adapted from the Ep 2 (Salem) tooling per
// creative-direction.md §16 — logic is unchanged; only captions_data.mjs's
// content differs per episode.
...
// moving (Margot, Werner, the bar patron, the Ketwurst vendor, Hanne, the
// TV broadcast, and the crowd chant all get this treatment).
...
async function qcOneClip(clip) {
  ...
  // Cap output to the source's frame-exact video-stream duration so
  // loudnorm's two-pass audio drift doesn't compound across 11 clips
  // (creative-direction.md §16 audio-hygiene rule).
