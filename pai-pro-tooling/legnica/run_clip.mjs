...
const BATTLEFIELD_BACKGROUND_RULE = `BATTLEFIELD BACKGROUND — PERMANENT RULE, applies to every soldier or crowd visible anywhere in this shot, foreground or background, named in this scene's own text or not. ROOT CAUSE, not just a symptom fix: a real camera can never actually see a soldier's back AND the front face of his actively-held shield at the same time — that combination is the single most common "medieval army" trope in training data for a completely different reason (a soldier MARCHING with a shield slung on his back, not fighting with one held forward), so describing a camera angle that shows soldiers' backs reliably produces that trope regardless of what the words say. COMPOSITIONAL RULE, not just a wording rule: NEVER stage a shot where a visible soldier's back is their dominant presented side to the camera. Every visible soldier is shown in PROFILE or THREE-QUARTER-FRONT relative to the camera — actively engaged, turning, reacting, glancing toward danger — never a static figure with a fixed back-to-camera pose. If the scene's own action would put a soldier's back toward the camera, that soldier must instead be actively turning/moving/reacting (which real chaos justifies anyway) so their back is never the shot's dominant, held view of them. Only once a soldier's front or three-quarter angle is toward the camera does a shield's painted face ever appear — held actively out in front of their body, never slung on their back.`;
