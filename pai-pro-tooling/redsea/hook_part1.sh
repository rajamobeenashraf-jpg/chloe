...
set -e; A=/home/user/pai-pro/projects/redsea/assets; F=/usr/share/fonts/X11/Type1/c0632bt_.pfb; in=$1; out=$2; HS=${3:-3.0}; FS=${4:-1.0}; TXT=${5:-$A/work/hook.txt}
dur=$(ffprobe -v error -show_entries format=duration -of csv=p=0 "$in"); fst=$(python3 -c "print(max(0,$dur-$FS))")
o="drawtext=fontfile=$F:textfile=$TXT:text_align=C:line_spacing=0:fontsize=66:fontcolor=black:box=1:boxcolor=black@1.0:boxborderw=26:x=(w-text_w)/2:y=150:enable='lt(t,$HS)'"
b="drawtext=fontfile=$F:textfile=$TXT:text_align=C:line_spacing=0:fontsize=66:fontcolor=black:box=1:boxcolor=white@1.0:boxborderw=20:x=(w-text_w)/2:y=150:enable='lt(t,$HS)'"
