...
Reused unchanged for Episode "The Fall of Constantinople 1453" (generic tool;
only the per-episode clip filename pattern below was adjusted).
...
def main():
    ...
    for clip in clips:
        ...
        # Episode-specific: a clip with an empty captions list has no
        # character/dialogue on screen (e.g. a dedicated wide impact insert)
        # -- nothing to transcribe or chunk, skip whisper entirely.
        if not clip["captions"]:
            all_out[cid] = []
            print(f"== {cid}: no dialogue (empty captions) -- skipped, 0 chunks")
            continue

        wav = f"/tmp/{cid}_chunks.wav"
        # Episode-specific: Constantinople clip files are named "<id>.mp4"
        # directly (e.g. clip01.mp4, clip13a-1.mp4) -- no "_v1" suffix like
        # Troy's assets/ dir used, since these are the final downloaded
        # deliverables, not versioned working renders.
        subprocess.run(
