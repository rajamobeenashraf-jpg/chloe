...
**Push-in mechanics, precisely measured (2026-08-30, targeted re-analysis of the same beat, isolating the camera move only):** this is NOT a physical dolly and must never be requested from the video model as one. Confirmed: zero parallax shift between the speaker/frozen figure/background as the shot progresses, uniform 2D scaling around a center point — this is a **digital/software zoom, i.e. a deterministic post-processing effect**, not a generated camera move. Exact arc: starts at waist-up/mid-shot framing → ends at chest-up medium-close-up framing; smooth digital ease-in, then a gentle ease-out/deceleration timed to land right as the punchline lands; no jitter, no handheld quality; total duration ~3.5-4.0 seconds. **Execution method: do not ask Seedance to perform this camera move.** Generate the beat as a near-static clip (only the speaking figure's face/mouth/blink/eyebrow animates; everything else, including the frozen figure, stays still per the freeze technique above), then apply the waist-up→chest-up zoom as a deterministic ffmpeg/PyAV post-process with the measured ease-in/ease-out timing. This is more precise than a generated push-in AND sidesteps any risk of the S14 static-pose-freeze failure mode, since the "camera work" carries zero model-obedience risk once it's a mathematical crop-and-scale operation on already-rendered footage.
