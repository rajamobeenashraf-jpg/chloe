...
| A shock/revelation beat WHERE THE FACE ITSELF REACTS (Part 12's actual SHOCK micro-sequence: eyes snap wide, lips part, total body freeze) | (universal row applies) | Hold 2-3s on the frozen face | Sudden push-in or crash zoom (menu item 6) | — |
| A crash zoom used for DEADPAN comedy instead — the face explicitly does NOT react (owner lock 2026-08-31, clarification found in the fifth audit pass: Escobar 4.3 uses the crash zoom on Pablo's established never-react deadpan law — "zero change in his face across the zoom" — which is the opposite of the SHOCK micro-sequence; the 2-3s hold above does not apply here, since there is no frozen-face reaction to hold on) | (universal row applies) | Governed by the shot's own duration budget, not a fixed hold — the "stillness inside the violence" is the joke, not an emotional freeze needing screen time | Crash zoom (menu item 6) | — |
...
12. **Single-subject slow push-in (realization/intimacy/gravitas)** — a smooth push toward one speaker with no second figure required and no freeze mechanic — distinct from #2 (which requires a frozen second figure) and #5 (which requires travel toward a third figure). This is the plain "slow push-in = intimacy/realization" move named in Part 12's emotion-to-camera map, given its own row here because it is one of the most-used techniques in practice (Escobar 1.3 — first realization the man by the pool is Pablo; 8.2 — the grave-line gravitas beat; 9.2 — the episode's single longest shot, the monologue). Gap closed 2026-08-30: this existed conceptually in Part 12 but had no row in this menu, so it was not actually part of the pre-generation checklist despite being used three times in one episode. **Shares the same mechanics and the same corrected execution method as #2 — see the "Push-in mechanics, precisely measured" entry above (Section D): explicitly decide and name ZOOM (zero parallax, uniform frame scaling) vs. DOLLY (real parallax, physical travel through space) rather than a generic "push," build it via the still-plus-annotated-camera-path method (annotate start/end framing on the reference still), and state the relevant stillness/no-extraneous-motion constraint as the prompt's first line even when no second figure needs to freeze — only the speaker's own performance and the camera should be moving.** Gap closed 2026-08-30 (second pass, owner-prompted verification): this item was added without that cross-reference on the first pass — caught only because the owner asked directly whether all of this session's learning had actually been applied, not by a self-initiated re-check. No shot has been generated yet using this technique (1.3/8.2/9.2 are all still pending), so this was a documentation gap only, not a production defect — but it would have caused one on the next push-in build if left as written.
