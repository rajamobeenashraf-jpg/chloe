...
- **Hazel-v5 Element (superseded, see v5.1 below):** `0dc0f31b-5fdc-4d85-bb2b-1a943eb3ca36` — built from the v5 canon 4K master + 4-view set (the same 5 images listed above).
- **Hazel-v5.1 Element (CURRENT, owner-accepted 2026-08-30):** `34bb316b-c5c4-4138-86ce-73bd96b80457`. Found immediately after standing up the plain v5-canon Element: generations from it consistently drifted toward a slimmer, more-sculpted face and a too-golden complexion — NOT what the v5 lock spec calls for (softly rounded/fuller cheeks, fair complexion with a rosy-pink undertone). ~8 correction rounds were tried (photo-edits with concrete magnitude language, reference-guided edits, fresh full-reference renders); the one that held both the correct complexion AND a real face-shape correction without breaking pose/wardrobe/background was accepted by the owner (2026-08-30) and added into the Element as a 6th reference image alongside the original 5 canon images. **This is now the PRIMARY identity reference for every new Hazel generation, project-wide** — embed `<<<34bb316b-c5c4-4138-86ce-73bd96b80457>>>` directly in the prompt as Hazel's grammatical subject/anchor. The frozen identity string above stays in every prompt too, as a secondary/belt-and-suspenders text description (left unedited/verbatim per the frozen-block rule — the correction lives in the Element's reference image, not in the text).
- Verified working on both `nano_banana_pro` (stills) and `seedance_2_5` (video, this episode's model) — test jobs `cb4f75b5` (still) and `113edb7c` (video, no first/last-frame drift), both against the original Hazel-v5 Element before the v5.1 correction. `pai-pro-tooling/alexander/build_prompt.mjs` implements this via `elementAnchorBlock`, now pointed at Hazel-v5.1.
