...
## Round 5 — 4 targeted corrections (owner feedback, 2026-08-20)
1. **Rewind card position.** "EARLIER TODAY" card moved from vertical-center to the upper safe zone (y=110 of 1280px). This is the single on-screen card element in the clip-2 scene; the owner's two bullets (card position + "earlier today" scene) both point at this one element, now fixed once.
2. **"…Revere rides tonight." still late.** Re-examined at 15ms window resolution — confirmed no earlier real onset exists before ~2.85s (pure noise floor beforehand), nudged start 2.96→2.85 and end 4.00→4.10 to fully bracket the real acoustic event (soft rise into the sharp -12dB cut-through spike at 3.44s) with zero margin trimmed off either end.
3. **"One if by land—/—two if by sea." — 4th attempt, root cause finally found.** Every-single-frame contact sheet (no skipping) revealed continuous mouth movement from ~5.5s to ~9.2s — but this is **breathless panting from the climb** (scripted as "both audibly out of breath"), not continuous speech; visual mouth-open was a contaminated signal for this clip all along, which is why 3 rounds of visual-anchoring kept failing. Pivoted back to pure acoustic evidence: the 6-8.5s region is confirmed pure breath noise (no distinguishable word-level burst even at low margin), with only ONE clear, sharp, isolated vocal event in the whole clip (8.56-9.20s, peak -15dB vs. -35 to -55dB breathing baseline). Re-anchored both lines tight against that single confirmed event — "One if by land—" 8.00-8.54 (immediately before, best-available placement, narratively consistent with the script's "as the second lantern settles" near-simultaneous staging), "—two if by sea." 8.56-9.20 (unchanged, the one strong anchor). Verified in final cut: dialogue now lines up with one-lantern-lit → both-lanterns-lit exactly as staged.
4. **Missing first "Evening."** Found a real, previously-missed isolated speech burst at 3.92-4.28s (peak -19dB) well before the existing cue's 7.52s start — matches the very first raw transcript captured for this clip months ago ("Evening. Evening. Just checking...") that got compressed to a single "Evening" in a later editing round. Restored as a separate first cue.

Also ruled out, for the record: checked whether the xfade/acrossfade assembly chain introduces any audio-vs-caption drift by comparing measured audio position in the final assembled cut against the source-clip analysis (clip1 and clip5 spot-checked) — confirmed no systematic pipeline desync; captions are burned onto each clip's own video track before assembly, so crossfading between already-muxed clips cannot desync a clip's internal audio-caption relationship. All timing issues were genuinely per-caption placement, not a pipeline bug.

Runtime unchanged: 127.2s.

## Round 5 — five surgical corrections (owner feedback, 2026-08-20)
1. **"EARLIER TODAY" card repositioned** from vertical-center to the upper safe zone (y=110 of 1280px frame) — clear of her face, addresses both bullet points in the request since they described the same on-screen element.
2. **"…Revere rides tonight." retimed** — ultra-fine 15ms-window envelope analysis nudged the start 0.11s earlier (2.85s) and widened the tail slightly (4.10s) to fully capture the isolated acoustic event.
3. **"One if by land— / —two if by sea." — 4th and final pass.** Root cause of 3 prior failed attempts identified: BOTH speakers are audibly out of breath from climbing per the script ("BOTH audibly out of breath the entire time"), so continuous mouth movement from panting was masquerading as continuous speech in visual inspection — invalidating that signal for this clip. Re-grounded purely in acoustic evidence: the only clear, isolated, loud vocal burst in the whole clip is 8.56-9.20s ("—two if by sea.", unchanged, well-evidenced); "One if by land—" has no acoustically distinguishable burst separate from breath noise, so it's placed immediately before the clear burst (8.00-8.54) per the script's own staging note ("as the second lantern settles" — implying near-simultaneous handoff, not the ~1.4s gap earlier rounds assumed).
4. **Missing first "Evening." restored.** The very first raw transcript captured for this clip (before later rounds condensed it) already showed "Evening. Evening. Just checking..." — confirmed for real via a distinct, isolated acoustic burst at 3.92-4.28s that had been present in the data all along but never captioned. Added as its own cue.
